import type { InventoryDetails } from "../hooks/use-inventory-details"
    ...
    <div className="border border-dashboard-border bg-white rounded-dashboard p-4">
      <div className="grid grid-cols-4 gap-x-8 gap-y-2 h-full">
        ...
        <div>
          ...
          <div className="font-medium text-gray-900">{details.customer?.name || details.customer || '-'}</div>
        </div>

        <div>
          <div className="text-sm text-gray-500 mb-1">Warehouse</div>
          <div className="font-medium text-gray-900">{details.warehouse?.name || details.warehouse || '-'}</div>
        </div>

        <div>
          <div className="text-sm text-gray-500 mb-1">SKU</div>
          <div className="font-medium text-gray-900">{details.sku}</div>
        </div>
        ...
        <div>
          ...
          <div className="font-medium text-gray-900">{details.pallet?.id || details.pallet || '-'}</div>
        </div>

        <div>
          <div className="text-sm text-gray-500 mb-1">Location</div>
          <div className="font-medium text-gray-900">{details.location?.name || details.location || '-'}</div>
        </div>
